...
const Preview = () => {
  ...
  const mutation = useMutation((newTodo) => {
    return axios.post(
      `${baseUrl}student/giveExam?id=${localStorage.getItem("examId")}`,
      newTodo,
      { headers: { "access-token": localStorage.getItem("userIn") } }
    );
  });
  const giveExam = () => {
    ...
    const final1 = final.map(
      (value) =>
        (value = Object.keys(value)
          .filter((key) => key !== "_id" && key !== "options")
          .reduce((obj, key) => {
            obj[key] = value[key];
            return obj;
          }, {}))
  ...
  useEffect(() => {
    ...
    mutation.isError
      ? console.log("error")
      : mutation.isSuccess && history("../allexam");
  }, [mutation]);
  // useEffect(() => {
